<button class="btn btn-success btn-sm btn-lg pull-right" wire:click="showForm"
  type="button">{{ trans('trans_parent.add parent') }}</button><br><br>
    ...
    <tbody>
      @foreach ($parents as $key => $value)
        <tr>
          ...
          <td>
            <button wire:click="edit({{ $value->id }})" title="{{ trans('trans_parent.edit') }}"
              class="btn btn-primary btn-sm"><i class="fa fa-edit"></i></button>

            <button type="button" class="btn btn-danger btn-sm" wire:click="delete({{ $value->id }})"
              title="{{ trans('trans_parent.delete') }}"><i class="fa fa-trash"></i></button>
          </td>
        </tr>
      @endforeach
  </table>
