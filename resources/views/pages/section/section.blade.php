...
<div class="row">
  <div class="col-md-12 mb-30">
    <div class="card card-statistics h-100">
      <div class="card-body">
        <a class="button x-small" href="#" data-toggle="modal"
          data-target="#exampleModal">{{ trans('trans_section.add section') }}</a>
      </div>
                              ...
                              <tbody>
                                @foreach ($s->sections as $key => $ss)
                                  <tr>
                                    ...
                                    <td>
                                      @if ($ss->status === 0)
                                        <label class="badge badge-success">{{ trans('trans_section.status active') }}</label>
                                      @else
                                        <label class="badge badge-danger">{{ trans('trans_section.status unactive') }}</label>
                                      @endif
                                  ...
                                  <!--تعديل قسم جديد -->
                                  <div class="modal fade" id="edit{{ $ss->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                      ...
                                      <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                          ...
                                          <form action="{{ route('section.update', [$ss->id]) }}" method="POST">
                                            ...
                                            <div class="row">
                                              <div class="col">
                                                <input type="text" name="name_ar" class="form-control" value="{{ $ss->getTranslation('name', 'ar') }}">
                                              </div>

                                              <div class="col">
                                                <input type="text" name="name_en" class="form-control" value="{{ $ss->getTranslation('name', 'en') }}">
                                              </div>
                                            </div>
                                            <br>
                                            <div class="col">
                                              ...
                                              <select name="stage_id" class="custom-select">
                                                @foreach ($stage as $st)
                                                  <option {{ $s->id == $st->id ? "selected" : "" }} value="{{ $st->id }}">
                                                    {{ $st->name }}
                                                  </option>
                                                @endforeach
                                              </select>
                                            ...
                                            <div class="col">
                                              ...
                                              <select name="grade_id" class="custom-select">
                                                ...
                                                <option {{ $ss->grade->id == $gr->id ? 'selected' : '' }} value="{{ $gr->id }}">{{ $gr->name }}</option>
                                                @endforeach
                                              </select>
                                            </div>
                                            <br>
                                            <div class="col">
                                                <input {{ $ss->status == 0 ? 'checked' : "" }} type="radio" id="active" name="status" value="0">
                                                <label for="active">{{ trans('trans_section.status active') }}</label>
                                                <span></span>
                                                <input {{ $ss->status == 1 ? 'checked' : "" }} type="radio" id="unactive" name="status" value="1">
                                                <label for="unactive">{{ trans('trans_section.status unactive') }}</label>
                                            </div>
                                        </div>
                                      ...
                                      <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                          <form action="{{ route('section.destroy', [$ss->id]) }}" method="post">
                                            ...
                                            <br><br>
                                            <div class="modal-footer">
                                              <button type="button" class="btn btn-secondary" data-dismiss="modal">{{ trans('trans_section.close') }}</button>
      ...
      <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title" style="font-family: 'Cairo', sans-serif;" id="exampleModalLabel">{{ trans('trans_section.add section') }}</h5>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
                ...
                <div class="row">
                  <div class="col">
                    <input type="text" name="name_ar" class="form-control"
                      placeholder="{{ trans('trans_section.name ar') }}">
                  </div>

                  <div class="col">
                    <input type="text" name="name_en" class="form-control"
                      placeholder="{{ trans('trans_section.name en') }}">
                  </div>
                </div>
                <br>
